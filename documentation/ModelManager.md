...
2) they have different meshes\
   they'll be drawn in different render calls\
   (unless it's the mesh vbo that is instanced but that's too complex for this rare situation)

So models should not share transformations. But other game objects can:

If a transformation should "attachParent" another transformation, implement FollowingTransformation
that indirects the master's matrix...